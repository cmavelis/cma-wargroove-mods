...
function Tornado:canExecuteWithTarget(unit, endPos, targetPos, strParam)
    ...
    if not Tornado.isInPreExecute then
        ...
        return targetUnit and not targetUnit.unitClass.isStructure and (not targetUnit.unitClass.isCommander or not Wargroove.areEnemies(targetUnit.playerId, unit.playerId))
    else
        ...
        for j, targetPos in pairs(targets) do
            ...
            for i, teleportPos in ipairs(teleportPositionsInRange) do
                if self:canSeeTarget(targetPos) then
                    local movingUnit = Wargroove.getUnitAt(targetPos)
                    if movingUnit and (Wargroove.getUnitAt(teleportPos) == nil) and Wargroove.canStandAt(movingUnit.unitClassId, teleportPos) and (teleportPos.x ~= pos.x or teleportPos.y ~= pos.y) then
            ...
            if u ~= nil then
                ...
                if not uc.isStructure and (not uc.isCommander or not Wargroove.areEnemies(u.playerId, unit.playerId)) and self:canSeeTarget(targetPos) then                    

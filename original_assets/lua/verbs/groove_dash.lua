...
function Dash:canExecuteAt(unit, endPos)
    ...
    local targets = self:getDashTargets(unit, endPos, {})
    return #targets > 0
end
...
function Dash:canExecuteWithTarget(unit, endPos, targetPos, strParam)
    ...
    local targets = self:getTargets(unit, Dash.lastLocation)
...
function Dash:getTargetsInDirection(unit, startingPos, jumpedUnits, direction)
    local targetPos = nil
...
function Dash:getTargets(unit, endPos, strParam)
    local processed = {}
    if Dash.isInPreExecute then
        local targets = self:getDashTargets(unit, Dash.lastLocation, Dash.jumpedUnits)
        for i, target in ipairs(targets) do
...
function Dash:getDashTargets(unit, startingPos, jumpedUnits)
    local targets = {}
    local leftTargets = self:getTargetsInDirection(unit, startingPos, jumpedUnits, {x = 1, y = 0})
    local rightTargets = self:getTargetsInDirection(unit, startingPos, jumpedUnits, {x = -1, y = 0})
    local upTargets = self:getTargetsInDirection(unit, startingPos, jumpedUnits, {x = 0, y = 1})
    local downTargets = self:getTargetsInDirection(unit, startingPos, jumpedUnits, {x = 0, y = -1})
        ...
        for i, hitPos in ipairs(hitPositions) do
            ...
            if u then
                if Wargroove.areEnemies(u.playerId, unit.playerId) then
                  local damage = Combat:getGrooveAttackerDamage(unit, u, "random", unit.pos, hitPos, path, nil) * 0.45
...
function Dash:generateOrders(unitId, canMove)
    ...
    for i, startPos in pairs(movePositions) do
        local targets = self:getDashTargets(unit, startPos, {})
    ...
    while #unfinishedDashes > 0 do
        ...
        local targets = self:getDashTargets(unit, path[#path], dash.jumpedUnits)
    ...
    for i, target in ipairs(targets) do
        ...
        for j, hitId in pairs(self:getUnitsFromHitPositions(hitPositions)) do
            ...
            if u ~= nil and Wargroove.areEnemies(u.playerId, unit.playerId) then
                local damage = Combat:getGrooveAttackerDamage(unit, u, "aiSimulation", unit.pos, u.pos, path, nil) * 0.45

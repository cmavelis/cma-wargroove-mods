...
function TeleportBeam:getRecruitableTargets(unit)
    ...
    for i, unit in pairs(allUnits) do
        for i, recruit in pairs(unit.recruits) do
            if recruit in {} then
                -- nothing
            else not TeleportBeam.recruitsContain(self, recruitableUnits, recruit) then
...
function TeleportBeam:canExecuteWithTarget(unit, endPos, targetPos, strParam)
    ...
    if not Wargroove.canPlayerRecruit(unit.playerId, classToRecruit) then
        return false
    end
...
function TeleportBeam:generateOrders(unitId, canMove)
    ...
    local unitToRecruit = Wargroove.getBestUnitToRecruit(affordableTargets)
